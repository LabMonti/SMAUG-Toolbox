...
function [RD,CD,order,TSaver] = ...
    ...
    disp('---BEGIN OPTICS ORDERING (using overlap distance)---');
    %Run optics algorithm to order points now that all neighborhoods have
    %been pre-calculated
    [RD,CD,order] = Sopticsv2_NbhdList(minPts,og_tr_ends);
    TSaver.Save('Optics');
    ...
    function [RD,CD,order]=Sopticsv2_NbhdList(minpts,og_tr_ends)
        ...
        for ii = 1:M
            ...
            nbhd = unique(nbhd);
            nbhd_length = length(nbhd);

            order(k) = ii; % object processed is true
            OP(ii) = true;

            %Find Core distance
            if nbhd_length < minpts
                CD(k) = 0;
            else
                %Calculate the distance between pairs of traces by only
                %using the points up to the length of the longer trace.
                %Assume we are using the manhattan metric in this case.  
                dist = zeros(nbhd_length,1);
                for jj = 1:nbhd_length
                    longer_length = max(og_tr_ends(ii),og_tr_ends(nbhd(jj)));
                    dist(jj) = sum(abs(points(ii,1:longer_length) - points(nbhd(jj),1:longer_length)))/longer_length;
                end
            ...
            if CD(k) > 0 %if core distance is not undefined
                ...
                while seeds.last_element > 0
                    ...
                    if nbhd_length < minpts
                        ...
                        dist = zeros(nbhd_length,1);
                        for jj = 1:nbhd_length
                            longer_length = max(og_tr_ends(currentp),og_tr_ends(nbhd(jj)));
                            dist(jj) = sum(abs(points(currentp,1:longer_length) - points(nbhd(jj),1:longer_length)))/longer_length;
                        end
