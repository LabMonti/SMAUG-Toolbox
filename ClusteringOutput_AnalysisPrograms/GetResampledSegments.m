%11May18 NDB: For an output file from segment clustering, re-samples all
%segments using a single grid of x-values and store the re-sampled segments
%back inside the OutputStruct
...
%         disp([abs(segment(1,1) - Xdist(xStartIndex)) xStep]);
%         disp([abs(segment(n,1) - Xdist(xEndIndex)) xStep]);
